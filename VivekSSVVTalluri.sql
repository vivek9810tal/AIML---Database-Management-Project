...
when 'M' then 'Mr.'
when 'F' then 'Ms.'
end ,' ',upper(CUSTOMER_FNAME), ' ',upper(CUSTOMER_LNAME)) as CUSTOMER_FULLNAME,
...
end) as CUSTOMER_CATEGORY from online_customer;
...
(p.PRODUCT_QUANTITY_AVAIL * p.PRODUCT_PRICE) as INVENTORY,
(case 
when p.PRODUCT_PRICE > 20000 then (0.8 * p.PRODUCT_PRICE)
when p.PRODUCT_PRICE > 10000 and p.PRODUCT_PRICE <= 20000 then (0.85 * p.PRODUCT_PRICE)
else (0.9 * p.PRODUCT_PRICE)
end) as NEWPRICE_AFTER_DISCOUNT from product p 
inner join order_items oi on p.PRODUCT_ID = oi.PRODUCT_ID  
group by PRODUCT_QUANTITY_AVAIL
...
count(pc.PRODUCT_CLASS_DESC) as COUNTS, sum((p.PRODUCT_QUANTITY_AVAIL * p.PRODUCT_PRICE)) as INVENTORY_VALUE
...
when 'M' then 'Mr.'
when 'F' then 'Ms.'
end ,' ',upper(oc.CUSTOMER_FNAME), ' ',upper(oc.CUSTOMER_LNAME)) as CUSTOMER_FULLNAME,
...
select distinct(s.SHIPPER_NAME), a.CITY, count(s.SHIPPER_NAME) as NUMBER_OF_CUSTOMER, count(oh.ORDER_STATUS) as NUMBER_OF_CONSIGNMENTS from order_header oh
inner join shipper s on oh.SHIPPER_ID = oh.SHIPPER_ID
inner join online_customer oc on oh.CUSTOMER_ID = oc.CUSTOMER_ID
inner join address a on a.ADDRESS_ID = oc.ADDRESS_ID
where s.SHIPPER_NAME = 'DHL' and oh.ORDER_STATUS = 'Shipped'
group by a.CITY; 
...
case when p.PRODUCT_QUANTITY_AVAIL = 0 THEN 'No sales'
	when pc.PRODUCT_CLASS_DESC in ('Electronics','Computer') then
		case when p.PRODUCT_QUANTITY_AVAIL <= (0.1 * (PRODUCT_QUANTITY)) then 'Low Inventory'
			 when p.PRODUCT_QUANTITY_AVAIL between (0.1 * (PRODUCT_QUANTITY)) and (0.5 * (PRODUCT_QUANTITY)) then ' Medium Inventory'
			 when p.PRODUCT_QUANTITY_AVAIL >= (0.5 * (PRODUCT_QUANTITY)) then 'Sufficient Inventory'
end 
	when pc.PRODUCT_CLASS_DESC in ('Stationery','Clothes') then
         case when p.PRODUCT_QUANTITY_AVAIL <= (0.2 * (PRODUCT_QUANTITY)) then 'Low Inventory'
              when p.PRODUCT_QUANTITY_AVAIL between (0.2 * (PRODUCT_QUANTITY)) and (0.6 * (PRODUCT_QUANTITY)) then 'Medium Inventory'
              when p.PRODUCT_QUANTITY_AVAIL >= (0.6 * (PRODUCT_QUANTITY)) then 'Sufficient Inventory'
         end
	when pc.PRODUCT_CLASS_DESC not in ('Stationery','Clothes', 'Electronics','Computer') THEN
         case when p.PRODUCT_QUANTITY_AVAIL <= (0.3 * (PRODUCT_QUANTITY)) then 'Low Inventory'
              when p.PRODUCT_QUANTITY_AVAIL between (0.3 * (PRODUCT_QUANTITY)) and (0.7 * (PRODUCT_QUANTITY)) then 'Medium Inventory'
              when p.PRODUCT_QUANTITY_AVAIL >= (0.7 * (PRODUCT_QUANTITY)) then 'Sufficient Inventory'
         end
end as INVENTORY_STATUS from PRODUCT p inner join PRODUCT_CLASS pc inner join ORDER_ITEMS oi
group by p.PRODUCT_ID;

/*Q7. Write a query to display order_id and volume of the biggest order (in terms of volume) that can fit in
carton id 10*/
select oi.ORDER_ID, (c.LEN * c.WIDTH * c.HEIGHT) as BIGGEST_VOLUME  from carton c, product p 
inner join order_items oi on p.PRODUCT_ID = oi.PRODUCT_ID
where c.CARTON_ID = 10 and (c.LEN * c.WIDTH * c.HEIGHT) < (p.LEN * p.WIDTH * p.HEIGHT * oi.PRODUCT_QUANTITY)
group by BIGGEST_VOLUME;
...
select p.PRODUCT_ID, p.PRODUCT_DESC, sum(oi.PRODUCT_QUANTITY) as TOTAL_QUANTITY from order_items oi
inner join product p on oi.PRODUCT_ID = p.PRODUCT_ID
inner join order_header oh on oi.ORDER_ID = oh.ORDER_ID
inner join online_customer oc on oh.CUSTOMER_ID = oc.CUSTOMER_ID
inner join address a on a.ADDRESS_ID = oc.ADDRESS_ID
where a.CITY not in 
(select CITY from address where CITY in ('Bangalore','New Delhi'))
group by oi.ORDER_ID
having PRODUCT_ID = 201
...
when 'M' then 'Mr.'
when 'F' then 'Ms.'
end ,' ',upper(oc.CUSTOMER_FNAME), ' ',upper(oc.CUSTOMER_LNAME)) as CUSTOMER_FULLNAME, sum(oi.PRODUCT_QUANTITY) as TOTAL_SHIPPED_QUANTITY
